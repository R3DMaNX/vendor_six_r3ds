...
func init() {
	android.RegisterModuleType("six_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = sixExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = sixExpandVariables(ctx, dep_file)
	...
	cmd := sixExpandVariables(ctx, String(g.properties.Cmd))
