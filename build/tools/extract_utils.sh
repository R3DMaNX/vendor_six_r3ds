...
function setup_vendor() {
    ...
    export SIX_ROOT="$3"
    if [ ! -d "$SIX_ROOT" ]; then
        echo "\$SIX_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$SIX_ROOT/$OUTDIR" ]; then
        mkdir -p "$SIX_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$SIX_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$SIX_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$SIX_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$SIX_ROOT"/vendor/six/build/tools/smali/baksmali.jar
        export SMALIJAR="$SIX_ROOT"/vendor/six/build/tools/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$SIX_ROOT"/vendor/six/build/tools/"$HOST"/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$SIX_ROOT"/vendor/six/build/tools/"$HOST"/compact_dex_converter
    fi
    ...
    if [ ! -f "$SIX_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$SIX_TARGET" >/dev/null; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$SIX_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$SIX_ROOT"/vendor/six/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$SIX_ROOT"/"$OUTDIR"/radio
